...
Several functions are supplied, with help pages, in the `disdat` package, to load the data into your workspace or get it for modelling:`disData`, `getDisData` and `getBorder`. The help files explain the nuances of these, and give examples of their application to load all data from all regions, or subsets of it. A function `predictors` returns the names of all predictor variables in the dataset.  

Finally, the function `spMapBook` enables you to generate maps, in PDF format, showing the location of PO and PA data for all species within one or more regions, with the "classic" theme looking like this:

```{r, out.width = 400, echo=F, fig.align="center"}
...
for(r in regions){
  ...
  presences <- getDisData(r, "train", "po")
  background <- getDisData(r, "train", "bg")
  ...
  for(s in species){
    ...
    if(r %in% c("AWT", "NSW")){
      sp_grp <- sp_presence$group[1]
      evaluation <- getDisData(r, "test", "env", sp_grp)
    } else{
      evaluation <- getDisData(r, "test", "env")
    }
    ...
    model_data <- pr_bg[, c("occ", predictors(r))]
    ...
    out_file$prediction <- predict(mod, evaluation, type = "prob")[,"1"]
...
for(i in sp_preds){
  ...
  if(r %in% c("AWT", "NSW")){
    sp_grp <- pred$group[1]
    evals <- getDisData(r, "test", "pa", sp_grp)
  } else{
    evals <- getDisData(r, "test", "pa")
  }
  n <- n + 1
